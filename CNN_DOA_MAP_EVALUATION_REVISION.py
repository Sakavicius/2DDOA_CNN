...
#%% evaluate noise influence on signal
plt.close('all')
T = 1
t = np.linspace(0,1,T*fs)
f = 1234
s = np.sin(2*np.pi*f*t)

S = np.abs(scipy.signal.stft(s,**stft_args)[2])[:,10]
s_mag = np.mean(np.power(S,2))
s_dB = 10*np.log10(s_mag)
...
n = (np.random.rand(t.shape[0])-0.5)*2*nA
n_mag = np.mean(np.power(n,2))
n_dB = 10*np.log10(n_mag)

n_scaled = n/n_mag
n_mag = np.mean(np.power(n_scaled,2))
n_dB = 10*np.log10(n_mag)

N = np.abs(scipy.signal.stft(n,**stft_args)[2])[:,10]

sn = s+n
SN = np.abs(scipy.signal.stft(sn,**stft_args)[2])[:,10]
...
snr = np.mean(np.power(n,2))/np.mean(np.power(s,2))
snr_db = 10*np.log10(snr)

SNR = np.mean(np.power(N,2))/np.mean(np.power(S,2))
SNR_DB = 10*np.log10(SNR)

print('signal SNR: {}, spectrum SNR {}'.format(snr_db,SNR_DB))
...
# ds_stft_filename = 'REVISION_TEST_DATASET_3SRC_STFTS_SNR_10_SPEECH_100.nc' # SPEECH
# ds_spos_filename = 'REVISION_TEST_DATASET_3SRC_SPOS_SNR_10_SPEECH_100.nc' # SPEECH
...
ds.to_netcdf('REVISION_ERRDS_ALL_STE.nc')
