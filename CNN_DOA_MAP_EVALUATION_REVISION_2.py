...
SPOS_TESTSPEECH = xr.load_dataarray('CONSISTENT_TEST_3SRC_SPOS_100.nc').values
...
for snr in [-30, -20, -10]:
    for signal_str in ['SPEECH','NOISE']:
        
        ds_stft_filename = 'REVISION_TEST_DATASET_3SRC_STFTS_SNR_{}_{}_100.nc'.format(snr,signal_str) 
        ds_spos_filename = 'REVISION_TEST_DATASET_3SRC_SPOS_SNR_{}_{}_100.nc'.format(snr,signal_str) 
        
        if signal_str == 'SPEECH':
            stft_calc_input_dir = os.path.join('D:/CNN3DFIELD_DATASETS',speech_dry_files_subdir) # SPEECH

        if signal_str == 'NOISE':
            stft_calc_input_dir = os.path.join('D:/CNN3DFIELD_DATASETS',noise_dry_files_subdir) # NOISE
...
#%%
SS = xr.load_dataset('REVISION_TEST_DATASET_3SRC_SPOS_SNR_10_SPEECH_100.nc')
SS = SS.to_array().values[0]

STFTS = xr.load_dataset('REVISION_TEST_DATASET_3SRC_STFTS_SNR_10_SPEECH_100.nc')
...
for model_filename in ['CNN_DOA_MAP_WANDB_CLEAN__3src_TESTBASELINE_COMPACT_NOTSHUFFLE_doaresxy_5_5_sigmaxy_20_20_model.h5']:
    ...
    for snr in [-30, -20, -10]:
        for signal_str in ['SPEECH','NOISE']:
            print("testing snr: {}, signal: {}".format(snr,signal_str))
            SS = xr.load_dataset('REVISION_TEST_DATASET_3SRC_SPOS_SNR_{}_{}_100.nc'.format(snr,signal_str))
            SS = SS.to_array().values[0]
            
            STFTS = xr.load_dataset('REVISION_TEST_DATASET_3SRC_STFTS_SNR_{}_{}_100.nc'.format(snr,signal_str))
            x = STFTS.to_array().values[0]
            ...
            # extract phase data
            # xphase = np.angle(xx)
            xx = np.vstack(x[:,:,:,:,0])[::skip]
            xphase = np.vstack(x[:,:,:,:,1])[::skip]
            xcplx = xx + xphase * 1.0j
            ...
            for i in tqdm(range(xphase.shape[0])):
            ...
            ds.to_netcdf('REVISION_3SRC_SPOS_SNR_{}_{}_resolution_{}_sigma_{}'.format(snr,signal_str,doares,sigma)+'CNN_eval_ds.nc')
            # wandb.save(os.path.split(curr_file)[-1]+'_ds.nc')
                        
            #  %%%% Visualize Model result
            if PLOTTING:
                slider_sample, slider_time = cube_show_slider( np.array(Y_PRED), np.array(Y_PRED), title='zxc')
            ...
            doa_SRP = pra.doa.SRP(mpos.T, config.fs, config.nfft, num_src=config.NUM_SIGNALS_TO_CONCATENATE, dim=3, n_grid=pt.shape[-2]*pt.shape[-1])
            ...
            for doa in baseline_doa:
                ...
                for i in tqdm(range(xcplx.shape[0])):
                    # print(i)
                    try:
                        # xxx = xx_test[i:i+num_samples_per_position,:,:]
                        # print(xxx.shape)
                        # doa.locate_sources(np.transpose(xxx,(1,2,0)), freq_bins=np.arange(20, 40))
                        baseline_doa[doa].locate_sources(np.transpose(np.array([xcplx[i]]),(1,2,0)),  freq_range=[500.0, 4000.0])
                ...
                ds['SNR'] = snr
                ERRDS.append(ds)
                ds.to_netcdf('REVISION_3SRC_SPOS_SNR_{}_{}_resolution_{}_sigma_{}'.format(snr,signal_str,doares,sigma)+'SRP_eval_ds.nc')
            #  %%%% Visualize Baseline
            BASELINE_ERRS = {}
        ...
        ERRDSNOYPRED = [DS.drop('Y_PRED') for DS in [DS for DS in ERRDS if 'SNR' in DS.keys()]]
        
        ERRDS_ALL = xr.concat(ERRDSNOYPRED,dim='sample')
        ds.to_netcdf('REVISION_ERRDS_ALL_SNR_-30_-10.nc')

#%%


df = ERRDS_ALL.where(ERRDS_ALL['SIGMA']==20,drop=True)[['ERRS','DOARES','METHOD']].to_dataframe()
df.groupby('METHOD').boxplot(by='DOARES')
#%%
import seaborn as sns
for s in np.unique(ERRDS_ALL.SNR.values):
    df = ERRDS_ALL.where(ERRDS_ALL['SNR']==s,drop=True)[['ERRS','DOARES','METHOD','SNR']].to_dataframe()
    sns.catplot(x='SNR', y='ERRS',
                     hue="METHOD",
                    data=df, kind="box");
...
for idx, m in enumerate(np.unique(ERRDS_ALL.METHOD.values)):
    print(idx)
    df = ERRDS_ALL.where(ERRDS_ALL['METHOD']==m,drop=True)[['ERRS','DOARES','SIGMA','SNR']].to_dataframe()
    z = sns.boxplot(x='SNR', y='ERRS',
                     hue="SIGMA",
                    data=df, ax=axs[idx],showfliers=False);
    axs[idx].set_ylim([0,180])
    axs[idx].set_xlabel('Signal-to-Noise Ratio, dB')    
    ...
    axs[idx].set_xticklabels([str(-float(i)) for i in labels])
    # plt.title('MAE of Predicted Source DoA, method='+str(m))
    z.get_legend().remove()
    title = 'CNN_DOA_MAP_3src_reso_{}_sigma_{}_SNR_-30_-10'.format(doares,sigma)
...
for idx, m in enumerate(np.unique(ERRDS_ALL.METHOD.values)):
    print(idx)
    df = ERRDS_ALL.where(ERRDS_ALL['METHOD']==m,drop=True)[['ERRS','SNR']].to_dataframe()
    mae_means = df.groupby('SNR').mean()
    MAE_MEANS.append(mae_means)
    
    #%%
    MAE_MEANS = pd.concat(MAE_MEANS,axis=1)
    MAE_MEANS.to_excel(title+'.xlsx')
